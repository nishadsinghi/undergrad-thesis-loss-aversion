...
def runMetropolis(chainIndex):
    ...
    # read observed data
    data = np.genfromtxt("../../src/dataZeroSure_cleaned_300_10500.csv", delimiter=',')[1:, 1:]
    ...
    class CostFunction:
        ...
        def __call__(self, parameters):
            ...
            for stakes in self.allStakes:
                ...
                observedPAccept = (np.shape(observedTrials[observedTrials[:, 0] == 1])[0])/numObservedTrials
                modelPAccept = (np.shape(modelTrials[modelTrials[:, 0] == 1])[0])/numModelTrials

                totalCost += (observedPAccept - modelPAccept)**2


            return (totalCost, parameters[3] - delta)

    costFunction = CostFunction(75, allStakes)
    bounds = ((0, 5), (0, 5), (0, 50), (0, 2), (0, 30), (-0.75, 0), (0, 50))   #decay, competition, noiseStdDev, nonDecisionTime, threshold, startingBias, constantInput
    ...
    SAVE_DIRECTORY = '../../data/parallel/threeBiasOnly/onlyStartingBias_dataset2_300_10500_POnly_{}'.format(str(bounds))
