...
def runMetropolis(chainIndex):
    ...
    def getStartingActivation(startingBias, threshold, startingVariability):
        randomNess = np.random.uniform(low=0, high=threshold*startingVariability, size=2)
        if startingBias < 0:
            activations = np.clip((np.array([-1*startingBias*threshold, 0]) + randomNess), 0, threshold)
            return activations
        else:
            activations = np.clip((np.array([0, startingBias*threshold]) + randomNess), 0, threshold)
            return activations
    ...
    lcaWrapper = lambda gain, loss, decay, competition, noiseStdDev, nonDecisionTime, threshold, startingBias, constantInput, startingVariability: \
        lca(attributeProbabilities, getChoiceAttributes(gain, loss), getStartingActivation(startingBias, threshold, startingVariability), decay, competition, constantInput,
    ...
    bounds = ((0, 5), (0, 5), (0, 50), (0, 2), (0, 30), (-0.5, 0), (0, 50), (0, 1))   #decay, competition, noiseStdDev, nonDecisionTime, threshold, startingBias, constantInput, startingVariability
    # meanStartingPointForSearch = np.mean(np.array(bounds), axis=1)
    ...
    SAVE_DIRECTORY = '../../data/parallel/threeBiasOnly/onlyStartingBiasWithVariability_dataset2_{}'.format(str(bounds))
