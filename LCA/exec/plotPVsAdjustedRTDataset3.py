...
data = np.genfromtxt("../../dataset3/dataset3_preprocessed.csv", delimiter=',')[1:, 1:]
...
def computeParticipantAllLogRTResidual(participantIndex):
    ...
    participantAllRT = participantData[:, 3]
    participantAllLogRT = np.log(participantAllRT).reshape(-1, 1)
    participantAllGainLoss = participantData[:, 1:3]
...
allParticipantIndex = list(set(data[:, 0]))
print(allParticipantIndex)
allParticipantMeanPAcceptForBinnedRT = np.array([computeParticipantMeanPAcceptForBinnedRT(participantIndex, 5) for participantIndex in allParticipantIndex])
...
startingActivations = [0, 0]
lcaWrapper = lambda gain, loss, decay, competition, noiseStdDev, nonDecisionTime, threshold, lossWeight, attribute1Prob, constantInput1, constantInput2: \
        lca((attribute1Prob, 1-attribute1Prob), getChoiceAttributes(gain, lossWeight*loss), startingActivations, decay, competition, (constantInput1, constantInput2),
            noiseStdDev, nonDecisionTime, getAllThresholds(threshold))
...
plotPAcceptVsAdjustedRT([1.21283573,  1.90145466, 31.09803827,  0, 15.22699408, 3.47883203,  0.54220646, 29.56671347, 31.36071942], "LCA")
