    ...
    def __call__(self, parameters):
        ...
        for stakes in self.allStakes:
            ...
            for choice in range(2):
                ...
                modelFrequencies = \
                np.histogram(modelRTsForChoice, bins=np.concatenate(([0], quantilesBoundaries, [100])))[0] * (numObservedTrials / numModelTrials)
...
costFunction = CostFunction(100, allStakes)

print(costFunction([2.06155217e+00,  3.18317118e+00,  2.08697307e+01,  1.27605889e-01, 1.04339815e+01, -1.95599890e-01,  3.01482276e+01]))
exit()

bounds = ((0, 5), (0, 5), (0, 75), (0, 2), (0, 75), (-1, 1), (0, 50))
meanStartingPointForSearch = np.mean(np.array(bounds), axis=1)
rangeOfParams = np.diff((np.array(bounds))).flatten()
initialParameterVariance = rangeOfParams/2.75
parameterVarianceMultiplier = np.array((0.99, 0.99, 0.99, 0.99, 0.99, 0.99, 0.99))
SAVEFILE = '../data/phase2/chiSquare_newData_frequencies_startingBias_{}.pickle'.format(str(bounds))
