...
actualData = np.genfromtxt("data_preprocessed.csv", delimiter=',')[1:, 1:]
allParticipants = set(actualData[:, 0])
...
def wrapper(data):
    allParticipantMeanPAcceptForBinnedRT = np.array([computeParticipantMeanPAcceptForBinnedRT(participantIndex, 5, data) for participantIndex in allParticipants])
...
fullModelSimulatedData = np.genfromtxt("simulatedData/fullModel.csv", delimiter=',')
...
plt.tight_layout()
plt.savefig("fig/5_behavMarkerObservationOnly.png", bbox_inches='tight')
plt.close()
...
noBiasSimulatedData = np.genfromtxt("simulatedData/noPredecisionalBias.csv", delimiter=',')
