...
# initialParameterVariance = (2.5, 2.5, 10, 0.25, 20, 20, 20)
#
# while True:
#     initialParameterValues = np.absolute(np.random.normal((5, 5, 25, 1, 20, 20, 50), initialParameterVariance))
#
...
try:
    saveFile = open('../data/fitLCAAllParametersForcedToBePositiveAndNonZeroStartingPoint.pickle', 'rb')
    ...
    parameterVarianceMultiplier = np.array((0.99, 0.99, 0.99, 0.99, 0.99, 0.99, 0.99))**(numIterationsPrevious//100)
    print("Num Iterations: ", numIterationsPrevious, " Multiplier: ", parameterVarianceMultiplier)
    parameterVariance = np.multiply(np.array((2.5, 2.5, 10, 0.25, 20, 20, 20)), parameterVarianceMultiplier)
except:
    exit()
...
for minimizationIteration in range(numIterationsPrevious, numIterationsMinAlgo+numIterationsPrevious):
    ...
    if minimizationIteration % 100 == 0:
        parameterVariance = 0.99 * parameterVariance

    # propose new parameters
    newParameters = np.absolute(np.random.normal(loc=bestParameters, scale=parameterVariance))
    ...
    saveFile = open('../data/fitLCAAllParametersForcedToBePositiveAndNonZeroStartingPoint.pickle', 'wb')
