...
class CostFunction:
    ...
    def __call__(self, parameters, lossWeight):
        cost = 0
        for gainValue in self.allGainValues:
            for lossValue in self.allLossValues:
                lcaWrapper = getLCAWrapperForLossWeight(lossWeight)
...
initialParameterVariance = (1, 1, 10, 0.25, 20, 20, 5)
...
while True:
    ...
    initialCost = costFunction(initialParameterValues[:-1], initialParameterValues[-1])
    if initialCost != -1:
        break
...
for minimizationIteration in range(numIterationsMinAlgo):
    ...
    # compute cost
    cost = costFunction(newParameters[:-1], newParameters[-1])
    if cost == -1:
        continue
    ...
    saveFile = open('../data/varyAssumptions/flexibleLossWeight.pickle', 'wb')
    pickle.dump(recordOfBestParameters, saveFile)
    saveFile.close()

    # time for iteration
    print("Iteration: {} ".format(minimizationIteration))
