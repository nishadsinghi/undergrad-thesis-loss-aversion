...
#
# while True:
#     initialParameterValues = np.random.normal((7.5, 7.5, 25, 1, 20, 20, 50), initialParameterVariance)
#
...
try:
    saveFile = open('../data/fitLCANonZeroStartingPoint.pickle', 'rb')
...
for minimizationIteration in range(numIterationsPrevious, numIterationsMinAlgo+numIterationsPrevious):
    ...
    if newParameters[6] < 0:
        newParameters[6] = -newParameters[6]
    ...
    saveFile = open('../data/fitLCANonZeroStartingPoint.pickle'.format(TERM1COEFF, TERM2COEFF, TERM3COEFF), 'wb')
